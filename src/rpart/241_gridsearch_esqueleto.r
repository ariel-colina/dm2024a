...
PARAM$semillas <- c(779173,816191,555337,529229,607531)
...
archivo_salida <- "./exp/HT2020/gridsearch2.txt"
...
for (vmax_depth in c(4, 6, 8, 10)) {
  for (vmin_split in c(1000, 800, 600)) {
    for (vminbucket in unique(as.integer(vmin_split*c(4/5,3/4,1/2,1/4,1/5)))) {
      for (mcp in seq(-0.75)) {
    # notar como se agrega

    # vminsplit  minima cantidad de registros en un nodo para hacer el split
    param_basicos <- list(
      "cp" = mcp, # complejidad minima
      "minsplit" = vmin_split,
    ...
    tb_grid_search <- rbindlist( 
      list( tb_grid_search, 
            list( vmax_depth, vmin_split, mcp, vminbucket, ganancia_promedio) ) )
      }
    }
